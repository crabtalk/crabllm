...
//!
//! OpenAI spells it `reasoning_effort` (an enum), Anthropic and Gemini spell it
//! as a token budget. The IR carries the effort level as canonical and treats
//! the budget as one expression of it, so the same knob survives any route.
use crabllm_core::{
...
fn ir_request(model: &str, thinking: Option<ir::Thinking>) -> ir::Request {
    ir::Request {
        model: model.into(),
        system: None,
...
fn a_budget_rounds_down_to_a_level() {
    assert_eq!(Effort::from_budget(8000), Effort::High);
    assert_eq!(Effort::from_budget(8192), Effort::XHigh);
    assert_eq!(Effort::from_budget(1024), Effort::Low);
    assert_eq!(Effort::from_budget(1023), Effort::Minimal);
    assert_eq!(Effort::from_budget(0), Effort::None);
}
...
fn every_level_round_trips_through_its_budget() {
    for level in [
        ...
        assert_eq!(Effort::from_budget(level.budget_tokens()), level);
...
#[test]
fn a_reasoning_effort_becomes_an_anthropic_budget() {
    let body = json!({
...
/// Anthropic rejects a request whose thinking budget meets or exceeds
/// `max_tokens`, so the budget is clamped rather than passed through.
#[test]
fn the_budget_is_clamped_below_max_tokens() {
    let mut req = ir_request(
        "claude-sonnet-4",
        Some(ir::Thinking {
            effort: Effort::Max,
    ...
    let out = anthropic::Request::from(&req);

    assert_eq!(out.thinking.expect("thinking").budget_tokens, Some(999));
}
...
fn a_gemini_thinking_budget_is_read_into_the_ir() {
    ...
    assert_eq!(thinking.effort, Effort::XHigh);
...
fn a_dynamic_gemini_budget_lands_on_the_default_level() {
    ...
    assert_eq!(thinking.effort, Effort::default());
